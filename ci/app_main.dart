  ...
  Future<void> _alert(String msg) async {
    await showDialog(
      ...
      builder: (_) => AlertDialog(
        title: const Text('No se puede importar'),
        content: Text(msg),
        actions: [
          TextButton(
          ...
          child: Column(
            ...
            children: [
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i){
                      ...
                      return CheckboxListTile(
                        ...
                        title: Text(name),
                        subtitle: Text('ID: ' + (id.isEmpty ? '(sin ID)' : id) + ro, maxLines: 1, overflow: TextOverflow.ellipsis),
                      );
                    },
                  ),
                ),
              ...
              Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                ...
                FilledButton.icon(
                  onPressed: () async {
                    final idsAll = _selected.entries.where((e)=>e.value).map((e)=>e.key).toList();
                    if (idsAll.isEmpty) { await _alert('Selecciona al menos un calendario.'); return; }
                    final validIds = idsAll.where((k)=>k.isNotEmpty).toList();
                    if (validIds.isEmpty) { await _alert('Los calendarios seleccionados no tienen un ID válido.'); return; }
                    if (_end.isBefore(_start)) { await _alert('El fin no puede ser anterior al inicio.'); return; }
                    Navigator.of(context).pop(ImportParamsMulti(calendarIds: validIds, start: _start, end: _end));
                  },
